...
var require_common = __commonJS({
  "../../../.yarn/__virtual__/debug-virtual-4a69f763f8/0/cache/debug-npm-4.4.0-f1ca372596-db94f1a182.zip/node_modules/debug/src/common.js"(exports, module) {
    "use strict";
    init_esm_shims();
    function setup(env) {
...
var require_browser = __commonJS({
  "../../../.yarn/__virtual__/debug-virtual-4a69f763f8/0/cache/debug-npm-4.4.0-f1ca372596-db94f1a182.zip/node_modules/debug/src/browser.js"(exports, module) {
    "use strict";
    init_esm_shims();
...
var require_node = __commonJS({
  "../../../.yarn/__virtual__/debug-virtual-4a69f763f8/0/cache/debug-npm-4.4.0-f1ca372596-db94f1a182.zip/node_modules/debug/src/node.js"(exports, module) {
    "use strict";
    init_esm_shims();
...
var require_src = __commonJS({
  "../../../.yarn/__virtual__/debug-virtual-4a69f763f8/0/cache/debug-npm-4.4.0-f1ca372596-db94f1a182.zip/node_modules/debug/src/index.js"(exports, module) {
    "use strict";
    init_esm_shims();
...
init_esm_shims();
import { collectDependencyVersions, createArtifact, getSdkVersion } from "@apps-in-toss/plugins";
...
async function clearBuildCache(packageRoot) {
  await fs.rm(path.join(packageRoot, DIST_DIRECTORY), { recursive: true, force: true });
}
async function ensureGraniteConfigExists(graniteConfigPath) {
  try {
    await fs.access(graniteConfigPath);
  } catch {
...
async function readAppManifest(appJsonPath) {
  const appJsonRaw = await fs.readFile(appJsonPath, "utf8");
...
async function resolveBuildStrategy(graniteConfigPath, strategyFactories) {
  const graniteConfigSource = await fs2.readFile(graniteConfigPath, "utf8");
...
async function prepareArtifact({
  ...
  const appJsonPath = path2.join(packageRoot, ".granite", APP_MANIFEST_NAME);
  const { appName, appType } = await readAppManifest(appJsonPath);
  const outfile = path2.join(packageRoot, `${appName}.ait`);
...
async function prepareBuild({
  ...
  const graniteConfigPath = path3.join(packageRoot, "granite.config.ts");
...
import { getPackageManager } from "@apps-in-toss/web-config";
...
async function createRuntimeBundleFiles({
  ...
  const distPath = path4.isAbsolute(bundleOutputDir) ? bundleOutputDir : path4.join(packageRoot, bundleOutputDir);
  const platforms = ["ios", "android"];
  const runtimeName = runtimeVersion.replace(/\./g, "_");
  const bundleFiles = [];
  for (const platform of platforms) {
    const sourceJsPath = path4.join(distPath, `bundle.${platform}.js`);
    const sourceMapPath = path4.join(distPath, `bundle.${platform}.js.map`);
    const targetJsPath = path4.join(distPath, `bundle.${platform}.${runtimeName}.js`);
    const targetMapPath = path4.join(distPath, `bundle.${platform}.${runtimeName}.js.map`);
...
var RNBuildStrategy = class {
  ...
  async build(input, context) {
    ...
    for (const definition of input.runtimeBuildDefinitions) {
      ...
      try {
        await execa(graniteCommand.executable, commandArgs, {
        ...
        if (tempConfigPath != null) {
          await fs4.rm(tempConfigPath, { force: true });
        }
      }
      results.push({
...
async function resolveGraniteBuildArgs(definition, context) {
  ...
  const tempConfigPath = path5.join(context.packageRoot, tempConfigRelativePath);
  try {
...
function createTempConfigRelativePath(targetVersion) {
  ...
  return path5.join(GRANITE_DIRECTORY, `${TEMP_CONFIG_FILE_PREFIX}-${runtimeName}.config.ts`);
}
async function writeRuntimeConfig(graniteConfigPath, tempConfigPath, targetVersion) {
  const source = await fs4.readFile(graniteConfigPath, "utf8");
  ...
  await fs4.mkdir(path5.dirname(tempConfigPath), { recursive: true });
  await fs4.writeFile(tempConfigPath, transformed.code, "utf8");
}
...
import { babelBuild, getPackageManager as getPackageManager2, loadConfig, writeBundle } from "@apps-in-toss/web-config";
...
var WebBuildStrategy = class {
  ...
  async build(input, context) {
    ...
    for (const definition of input.runtimeBuildDefinitions) {
      for (const platform of ["ios", "android"]) {
        ...
        await writeBundle(buildResult, { platform, config, cwd: context.packageRoot });
      }
      results.push({
        runtimeVersion: definition.runtimeVersion,
        bundleFiles: await createRuntimeBundleFiles({
  ...
  async getArtifactOptions(context) {
    ...
    const webDistDir = path6.join(context.packageRoot, config.outdir, "web");
  ...
  async ensurePrepared(context) {
    ...
    const outdir = path6.join(context.packageRoot, config.outdir);
    const webDistDir = path6.join(context.packageRoot, config.outdir, "web");
    try {
      const files = await fs5.readdir(webDistDir);
    ...
    const { packageManager } = getPackageManager2({ isExecutor: true });
    await fs5.rm(outdir, { recursive: true, force: true });
    await execa2(packageManager, config.web.commands.build.split(" "), {
      cwd: context.packageRoot,
      stdio: "inherit",
      shell: true
    });
    await fs5.mkdir(webDistDir, { recursive: true });
    const items = await fs5.readdir(outdir);
    ...
    const webDistFiles = await fs5.readdir(webDistDir);
  ...
  async writeAppJson(context) {
    const config = await this.ensureConfig();
    const graniteDir = path6.join(context.packageRoot, ".granite");
    await fs5.mkdir(graniteDir, { recursive: true });
    await fs5.writeFile(
      path6.join(graniteDir, APP_MANIFEST_NAME2),
      JSON.stringify(
        {
...
async function buildRuntimeBundle({
  ...
  const projectRequire = createRequireFromPath(path6.join(packageRoot, "package.json"));
...
async function getWebDistFiles(webDistDir) {
  ...
  try {
    const allFiles = await fs5.readdir(webDistDir, { recursive: true });
    const webFiles = await Promise.all(
      allFiles.map(async (file) => {
        const filePath = path6.join(webDistDir, String(file));
...
var BuildCommand = class extends Command {
  ...
  async execute() {
    ...
    p3.log.success(`AIT build completed (${path7.basename(buildResult.outfile)})`);
...
import { getPackageRoot as getPackageRoot2, readZipContent } from "@granite-js/utils";
...
var TokenStorage = class {
  static get path() {
    ...
    const dir = path8.join(home, ".ait");
    if (!fs6.existsSync(dir)) {
      fs6.mkdirSync(dir, { recursive: true });
    }
    return path8.join(dir, "credentials");
  }
  static read() {
  ...
  static write(map) {
    ...
    fs6.writeFileSync(file, content, { encoding: "utf8" });
  }
...
function uploadArtifactToRemote() {
  return async (params) => {
    ...
    const stat = await fs7.promises.stat(config.artifactPath);
    const stream = fs7.createReadStream(config.artifactPath);
...
var DeployCommand = class extends Command2 {
  ...
  async execute() {
    ...
    try {
      ...
      const buffer = fs8.readFileSync(resolvedArtifactPath);
      ...
      const { appName, deploymentId } = await (async () => {
        if (format === AppsInTossBundle.Format.AIT) {
          ...
          assert(typeof appName2 === "string", "ait \uD30C\uC77C\uC5D0 appName\uC774 \uC5C6\uC2B5\uB2C8\uB2E4");
          assert(typeof deploymentId2 === "string", "ait \uD30C\uC77C\uC5D0 deploymentId\uAC00 \uC5C6\uC2B5\uB2C8\uB2E4");
          ...
          const rawAppManifest = await readZipContent(resolvedArtifactPath, APP_MANIFEST_NAME);
          ...
          assert(typeof appName2 === "string", "app.json \uD30C\uC77C\uC5D0 appName\uC774 \uC5C6\uC2B5\uB2C8\uB2E4");
          assert(typeof deploymentId2 === "string", "app.json \uD30C\uC77C\uC5D0 deploymentId\uAC00 \uC5C6\uC2B5\uB2C8\uB2E4");
...
`);
  ...
  getArtifactPath(location) {
    if (location) {
      ...
      return path9.resolve(location);
    }
    const packageRoot = getPackageRoot2();
    const artifactFile = fs8.readdirSync(packageRoot).find((file) => file.endsWith(".ait"));
    if (!artifactFile) {
      throw new Error("\uBC30\uD3EC\uD560 .ait \uD30C\uC77C\uC744 \uCC3E\uC744 \uC218 \uC5C6\uC2B5\uB2C8\uB2E4.");
    }
    return path9.resolve(packageRoot, artifactFile);
  }
};
...
async function templateWebFramework({ appName, cwd, skipInput }) {
  const packageJson = await fs9.promises.readFile(path10.join(cwd, "package.json"), {
    encoding: "utf-8"
  });
  ...
  await fs9.promises.writeFile(path10.join(cwd, "package.json"), JSON.stringify(scripts, null, 2), {
    encoding: "utf-8"
  });
  p5.log.step(".gitignore \uD30C\uC77C\uC744 \uC5C5\uB370\uC774\uD2B8\uD558\uB294 \uC911...");
  await fs9.promises.appendFile(path10.join(cwd, ".gitignore"), "\n.granite\n");
  const placeholder = {
  ...
  await fs9.promises.writeFile(path10.join(cwd, "granite.config.ts"), config, {
    encoding: "utf-8"
  });
}
async function templateReactNative({ appName, cwd }) {
  const packageJsonPath = path10.join(cwd, "package.json");
  const packageJsonRaw = await fs9.promises.readFile(packageJsonPath, {
    encoding: "utf-8"
  });
  ...
  await fs9.promises.writeFile(packageJsonPath, JSON.stringify(packageJson, null, 2), {
    encoding: "utf-8"
  });
  p5.log.step("granite.config.ts \uD30C\uC77C\uC744 \uC0DD\uC131\uD558\uB294 \uC911...");
  ...
  await fs9.promises.writeFile(path10.join(cwd, "src", "_app.tsx"), APP_TEMPLATE, {
    encoding: "utf-8"
  });
}
var InitCommand = class extends Command3 {
  ...
  template = Option2.String("--template", {
    required: false,
...
async function renameFile(oldPath, newPath) {
  try {
    await fs10.rename(oldPath, newPath);
  } catch {
...
var MigrateCommand = class extends Command4 {
  ...
  target = Option3.String({ required: false });
...
import { Command as Command6, Option as Option5 } from "clipanion";
...
async function extractSentryDebugId(bundlePath) {
  const bundleContent = await fs13.readFile(bundlePath, "utf-8");
...
// src/integrations/UploadSentrySourcemap/UploadSentrySourcemap.ts
var UploadSentrySourcemap = class extends Command6 {
...
// src/index.ts
import { babelBuild as babelBuild2, babelJsonReplacementPlugin, getPackageManager as getPackageManager3, loadConfig as loadConfig2 } from "@apps-in-toss/web-config";
function initialize() {
  const cli = new Cli({
    binaryLabel: "ait",
    binaryName: "ait"
...
export {
  babelBuild2 as babelBuild,
  babelJsonReplacementPlugin,
  getPackageManager3 as getPackageManager,
  initialize,
